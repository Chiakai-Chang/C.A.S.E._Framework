...
9/11 最新驗收：另一個正常任務雖被流程標為完成，獨立評分仍發現一項錯誤，兩層核對皆漏驗。先前指定主案例通過的紀錄保留，但有效引用不保證判斷正確，尚不宜無人監督執行重要工作。本批未合併／推送。[最新失敗與完整證據](docs/evaluation/2026-09-11-holdout-journey-report.md)

## 它解決什麼問題？

遇到符合安全條件的核對連線中斷，pi 流程會在原預算內自動換一次對話接續，保留已完成成果；寫入中斷或安全狀態不明則不盲目重跑。[接續規則](workflow-kit/docs/V2.md)

目前改善重點是把既有返修與跨對話接續用完整旅程驗清楚，不另加固定角色。已修正過期仍呼叫模型的浪費；語意漏驗尚未解決。[目前已做與尚待完成](docs/design/2026-09-11-goal-loop-convergence.md)
...
2. **清楚交辦。** 規劃者拆出必要工作包。執行者不承接前一角色的對話；pi 的一般來源預設提供版本化索引、按需讀取，明確要求附全文的材料仍保留。pi 也會載入適用的使用者與專案指引，不是完全空白的 context。
...
- **後續修正與實測：** 9/11 正常主案例已從空白成果完成整案；引用介面亦有後續局部驗證。這些結果不代表異議路徑已通過，亦非品質或成本優勢的證明。[驗證範圍與證據索引](workflow-kit/docs/READINESS.md)保留所有成功、失敗及適用限制。
- **接續與異議的歷史缺陷：** 9/9 曾發生成果修好、獨立核對通過，卻遭整體驗收誤判而受阻。[當時結果](docs/evaluation/2026-09-09-repaired-review-report.md)。後續已實作異議及混合回覆拒收機制，但真實模型的異議路徑尚未驗證成功；不把一般整案成功當成異議成功。[設計與邊界](docs/design/2026-09-10-review-dispute-boundary.md)
...
執行紀錄可能包含讀取內容、檢查輸出與模型回覆；分享卷宗或上傳 repository 前須檢查私密資料，框架不保證自動去敏。
