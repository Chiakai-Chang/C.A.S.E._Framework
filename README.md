...
**使用前知道限制：** 模型曾把錯誤成果誤判為通過；分開 context、有效引用或多次核對都不是正確性保證。重要工作應接上適用的實際檢查，並保留必要的人工把關。[功能與驗證範圍](workflow-kit/docs/READINESS.md) · [交付狀態](docs/STATUS.md)
...
| 核對時連線失敗，或期限已到仍繼續耗費 | 符合安全條件時換新 session 接續一次；原預算跨執行累計，過期在呼叫模型前停止 |
