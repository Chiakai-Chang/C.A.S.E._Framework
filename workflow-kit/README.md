...
pi 現可在原預算內自動恢復一次符合安全條件的唯讀核對連線中斷，不重做執行者成果。不是所有中斷都會重試，也不保證語意驗收正確。[接續規則](docs/V2.md)

接續沿用同一卷宗與原預算，過期會在模型呼叫前停止。已有返修及接續的工程驗證，但不能將它當成語意驗收可靠度；[目前改善與限制](docs/READINESS.md)分開記錄。
