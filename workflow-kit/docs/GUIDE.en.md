...
September 10 engineering update: mixed planner replies are now rejected at SDK and custom-runner return boundaries. A blocked reply must contain only blocked.reason; discovery decisions may still include a plan amendment. Rejected tool replies can be corrected in the same session. All 265 tests pass; no new model run was performed. Evidence-backed dispute handling without unnecessary artifact changes is designed but not implemented; see [scope and design](../../docs/design/2026-09-10-review-dispute-boundary.md).

September 9 model result: the repaired artifact passed independent review, but integration invented three defects contradicted by its own source reads. The planner repeated them and incorrectly reported no write scope; the diagnostic stopped after about 409 seconds without closing the case. The artifact remains correct. This synthetic continuation is not a complete original-budget run or evidence of general reliability. See [results and limits](../../docs/evaluation/2026-09-09-repaired-review-report.md); the earlier [repair-setting pair](../../docs/evaluation/2026-09-09-repair-thinking-report.md) remains separately recorded.
...
Latest verification: 259 local tests passed. In the fixed planning-handoff pair, planning took 49.461 seconds with the change versus 521.345 seconds for the baseline, but neither arm produced the deliverable or completed the workflow. The new worker hit a response-length limit, returned a planner-shaped result, was rejected for a missing file, and reread supplied sources without writing. No holdout or replacement attempt followed. This is a planning-stage signal, not evidence of improved task completion; see [results and limitations](../../docs/evaluation/2026-09-08-planning-handoff-report.md). SDK usage during cancellation may be incomplete, including reported zero usage for an active worker.

The September 8 read-receipt change passed 256 local tests and package dry-run (34 files). Its finite local-model batch completed without replacement attempts: the read/write/read-back probe passed; both full CASE arms timed out. Old-format output matched the oracle but the workflow was incomplete; the new-format arm produced no artifact. Constraints and traces were complete for both. No holdout was triggered. This does not demonstrate improved task quality. See the [full evidence and interpretation](../../docs/evaluation/2026-09-06-read-receipt-report.md); the earlier CI and comparisons below cover earlier versions.
