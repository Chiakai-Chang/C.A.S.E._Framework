...
**2.0.0-preview.1 — status as of September 10, 2026.** Workers now have version-checked literal editing for localized repairs. The full suite passed 307 tests with two Windows symlink-permission skips. A focused local-model repair completed in 40.206 seconds with every required field correct. The latest normal workflow still failed: whole-file repair fixed a boolean but dropped a previously correct label, and the second review was cancelled at the original 600-second limit (601.698 seconds including cleanup). No integration completed. This batch is not merged or pushed.
