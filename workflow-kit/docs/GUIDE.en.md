...
**2.0.0-preview.1 — status as of September 11, 2026.** Initial planning now has phase-specific guidance and an explicit boundary between production packets and built-in reviews. The full kit suite passed **322 tests with two Windows symlink-permission skips**; the package preview contains 36 files. The batch remains on its working branch, not merged or pushed.
...
The second case then resumed natively with its original plan and remaining budget: 92.540 seconds had already been recorded, leaving 507.460 seconds. Continuation ended at the reviewer without integration (509.608 seconds including outer overhead/cancellation). The artifact had seven incorrect fields; sources were preserved. This batch used **489,969 cumulative SDK tokens including cache** across both planning probes and continuation. No answers were injected, no artifact was imported, and the budget was not reset. The next investigation concerns worker role confusion, source-fact errors and review cost—not identical reruns or more generic reminders. Use the preview for versioned assignments, continuation or review records, not as a proven unattended worker. [Validation history](READINESS.md) retains the earlier imported-artifact success and normal-run failures separately.
