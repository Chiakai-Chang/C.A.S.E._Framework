...
**2.0.0-preview.1 — September 12, 2026.** Portable skills and durable records are available; pi additionally provides automatic isolated sessions. The latest full local-model task produced correct multi-source order data and a report in 361.349 seconds / 159,925 cumulative SDK tokens, with two dependent packets, six role sessions and no human intervention. Independent grading checked all fields and unchanged sources. This single existing fixture is not a blind benchmark or evidence of general superiority. Earlier semantic false acceptance remains unresolved; use suitable executable checks and human oversight for important work. Current regression: 351 passed, two permission skips, zero failures. [Evidence and scope](../../docs/evaluation/2026-09-12-push-acceptance-report.md) · [Git delivery](../../docs/STATUS.md).

The following dated results preserve development context; they do not supersede this current scope.

Native pi `stopReason:error` replies now stop as provider errors instead of triggering JSON-format correction. For `openai-completions`, a session-local fetch wrapper preserves structured connection-stage rejection codes before provider normalization loses them; it never parses error text or changes global fetch. Receiving an HTTP response clears that evidence, so authentication/quota responses do not trigger recovery. Stream failures after headers remain outside this recovery path. A real pi 0.84.2 SDK probe with substituted HTTP verified the mapping with one transport attempt, zero format corrections and no model calls.

Read-only reviewer/integrator connection failures can now recover automatically once per case, across runs and within existing budgets. The SDK must attest successful stop/cleanup, no executable checks, no pending tools and no accepted result. Only explicit `ECONNRESET`, `ETIMEDOUT`, `ECONNREFUSED` or `EAI_AGAIN` errors qualify. The failed session and costs remain recorded; a fresh session repeats only the interrupted review, not worker writes. Cancellation, semantic rejection, disputed rechecks, worker failures and unknown safety state are excluded. Custom transports without this evidence retain the previous stop behavior; supplying it carries responsibility for that trust boundary.

Continuation retains the same case and budget. Both cumulative runtime and the core wall-clock deadline since the first worker start apply; pauses do not extend the latter. The runner checks remaining time before starting and before each model call, preserving artifacts on expiry. A real-command test covers rejection, repair, interrupted integration and disk-backed continuation without repeated worker writes; role responses are controlled test doubles, not new model-quality evidence. Semantic false acceptance remains unresolved. See [current scope](READINESS.md).

**Earlier alternate full task: failed with false completion.** On fixed revision `154edb3`, four roles finished in **343.108 seconds / 220,286 cumulative SDK tokens** and marked the case completed, but independent grading found one incorrect field: nativeAutomaticHosts omitted pi. Both review stages missed it. Seven accepted citations were genuine; four fabricated IDs were rejected before correction. Source files were preserved. Two malformed path requests failed without returning file contents but remain scope violations under the existing evaluator. One fabricated ID contained an exact source hash from the earlier task; its origin is unresolved, not a confirmed cache defect. No production code, service settings or original budget changed. Do not rely on unattended acceptance for important work. [Full evidence and next investigation](../../docs/evaluation/2026-09-11-holdout-journey-report.md).

**Receipt-based review update:** Native pi review/integration now requests evidence as `{assessment, receiptIds}`. The SDK resolves current-session `case_read` IDs to actual source metadata, rejecting unknown, out-of-range or stale references. A passing claim needs at least one citation; a negative claim may use none when material is unavailable (up to 16 distinct IDs per evidence field). Saved evidence uses `{assessment, citations}`; `rawResultText` preserves the original submission. Both tool and final-text replies are checked. This certifies provenance, not relevance or correctness of prose; no new roles or budgets. Standalone SDK calls default to legacy mode; custom transports remain responsible for their own validation. Core/CLI records and dispute formats are unchanged. A targeted local-model integrator probe correctly identified one defect and one valid field in **42.792 seconds / 10,066 cumulative tokens**, with valid references. It was not a whole-workflow or efficiency comparison. Final regression: **333 passed, two permission skips**. [Evidence and limits](../../docs/evaluation/2026-09-11-receipt-review-report.md).

**Earlier main task: passed.** After the presentation change, a fresh blank-output run completed planner, worker, reviewer and integrator in **547.780 seconds**, within the unchanged 600-second budget. Every artifact field and source/scope check passed, with zero human interventions and no compaction. Usage was **454,646 cumulative SDK tokens including cache**; together with the standalone diagnostic, this batch used **575,000**. Review prose still contained incorrect line numbers or hash prefixes: artifact correctness and source integrity were independently checked by code, not established by trusting that prose. This is one successful task, not universal reliability, citation accuracy or proven efficiency. [Full evidence and limits](../../docs/evaluation/2026-09-11-worker-grounding-report.md).

**2.0.0-preview.1 — status as of September 11, 2026.** Initial planning has phase-specific guidance; pi worker assignments now use execution-oriented sections, including repair assignments. Core JSON, stored state, source versions, policies, feedback, acceptance and permissions are preserved; the rendered assignment is also checked against the original context-character limit. For current Git delivery, see STATUS above.

An offline audit verified 26 source read/search results from the failed worker against frozen sources; the first wrong-role reply preceded compaction. A separate SDK worker diagnostic using the complete original task with sectioned handoff produced a fully correct artifact in **169.286 seconds**, using **120,354 cumulative SDK tokens including cache**, with no compaction or wrong-role packet reply. This is one diagnostic, not a causal comparison or a complete workflow result. Its first launch failed before generation because the diagnostic omitted an onStart callback (zero SDK-reported tokens); that failure is retained. [Source-use diagnosis and limitations](../../docs/evaluation/2026-09-11-worker-grounding-report.md).

Two planning diagnostics are retained: the first took **260.642 seconds** and produced a redundant review packet that acquired write scope during format repair; the second took **93.554 seconds** and produced one production packet, but one check contradicted the permitted output read. Neither schema acceptance nor faster planning establishes complete semantic correctness. See [the new evidence and diagnosis](../../docs/evaluation/2026-09-11-initial-planning-report.md).

The second case then resumed natively with its original plan and remaining budget: 92.540 seconds had already been recorded, leaving 507.460 seconds. Continuation ended at the reviewer without integration (509.608 seconds including outer overhead/cancellation). The artifact had seven incorrect fields; sources were preserved. That earlier batch used **489,969 cumulative SDK tokens including cache** across both planning probes and continuation. No answers were injected, no artifact was imported, and the budget was not reset. Use the preview for versioned assignments, continuation or review records, not as a proven unattended worker. [Validation history](READINESS.md) retains earlier successes and failures separately.

The built-in pi workflow now supplies `verificationPaths` (deliverables and required inputs). Before a reviewer claims pass or an integrator marks any criterion passed, the SDK requires a successful `case_read` of each file in that session with a still-current source hash. Valid ranges and genuine empty files count; listing, search alone, out-of-range reads, stale versions and narrated claims do not. Rejection uses the existing session budget; negative reports remain possible. This minimum acquisition gate does not prove full coverage, retained context, understanding or factual citations. Custom `runSession` implementations, standalone SDK calls without the list, and portable core/CLI use do not automatically enforce it.
...
Fresh sessions do not inherit another role's conversation history. Pi still loads applicable user and project instructions, including context files from its agent directory and ancestor directories; this is not an empty context. Run records may contain source text, check output and model replies. Inspect private data before sharing or uploading them; CASE does not guarantee automatic redaction.

## V2 preview: contracts and fresh sessions

Initial pi planning receives assignment-specific system and result-tool guidance, rather than later dispute-handling instructions. Whole-task reading, production and acceptance obligations must still be assigned completely. Each packet is a production assignment; the runner already provides an independent reviewer per packet and whole-contract integration. Do not add packets solely to repeat those reviews; explicitly required report/evidence deliverables may still be separate work. `case_list` provides an on-demand material index, not automatically attached source bodies; source inspection remains available for dependencies, feasibility or missing inputs that prevent an actionable assignment. Permissions, validation and budgets are unchanged. This routing does not guarantee model compliance; direct SDK calls without the initial-phase marker retain the existing planning/feedback guidance.

`case_read` takes one file per call. A missing-file error explains the expected path and suggests `case_list`; it never unquotes, splits a list, or redirects to another file. After an accepted result, cancellation or turn-limit stop, the pi integration disables future compaction and cancels active compaction in the session's in-memory settings only. A late-created compaction controller is cancelled after its start notification; ordinary active-session compaction is preserved. This relies on SDK compaction methods/events (the pi 0.84.2 ordering was inspected), not a guarantee of zero additional provider requests or charges.

Workers can use `case_edit` for a localized repair: supply `path`, the current read receipt's `sourceSha256`, unique literal `oldText`, and `newText`. Stale versions, absent or repeated matches make no change. It only edits existing UTF-8 files within the original writeScope, capped at 1 MiB; whitespace and line endings match exactly. Receipts include before/after hashes. This preserves unrelated text, not semantic correctness, and is neither an OS sandbox nor a cross-process atomic transaction. The model may still choose full-file writing.

Invalid plan references report the packet, field/check and allowed contract IDs. `constraintIds` refers to constraints; `checks[].criterionIds` refers to acceptance criteria. The model must choose the applicable IDs; the core does not silently repair or weaken the plan. Source `delivery` belongs inside each `inputs[]` item, not at the result or packet level.

Pi sessions also expose `case_search` for case-sensitive literal lookup in one already-readable file. Use `{path,query,startLine?,maxMatches?}` to locate disputed values, then read surrounding context. Queries are single-line strings up to 256 UTF-16 units; files are capped at 1 MiB, pages at 20 matches and 24,000 UTF-16 units. Each match contains a complete line and its line number, alongside the source hash and explicit pagination. No-match applies only to that file from the selected start line, not the entire project or semantic truth. No shell or write permissions are added. Search query/content are not copied into trace metadata, though ordinary tool evidence can contain them.

### Evidence-backed disagreement with integration

An integrator's claim is feedback, not automatically a fact. On a failed semantic integration, the planner may inspect the disputed sources and submit a mutually exclusive `reviewDispute` reply instead of changing correct artifacts. It contains `reason`, exactly the failed `criterionIds`, and citations with `path`, the current `sha256`, one-based `startLine`/`endLine`, and the full selected `quote` (LF between lines). Only declared, current source/artifact files are allowed: up to 16 citations, 4,000 characters per quote, regular files up to 1 MiB. The core verifies provenance and literal matches, not whether the argument is semantically correct.

An accepted request starts one fresh integrator session that still checks all acceptance criteria. It cannot bypass approved executable checks, expand permissions, or declare completion itself. Genuine defects still require authorized repair; no evidence means no dispute shortcut. A planner's read-only tools do not remove the workers' declared write authority.

The same saved version and failed criterion can use this recheck only once, within the original total budget. Run records retain `reviewDisputes` and `pendingReviewDispute`. A `prepared` request may continue; an `issued` request whose result is unknown cannot silently run again. A rejected recheck preserves the findings and stops pending resolution. After inspecting the evidence, an explicit existing `retry`, `amend_plan`, or `revise` action changes the case revision and supersedes the older request without erasing costs. Do not manually edit run JSON or reset budgets to force another verdict. See the [exact contract](../skills/case-workflow/references/v2-contracts.md) and [current validation limits](READINESS.md).

Planning-handoff change: the pi planner's `case_list` returns a shallow index of names, kinds and byte sizes, without reading bodies (300 entries / 24,000 UTF-16 units; oversized results are rejected). It is not a content summary or new authority. Planning should establish actionable work and verification methods, not promote its inferred answers into acceptance requirements. Necessary feasibility/dependency checks remain allowed. Shared context clarifies that `requiredMaterials` already contains full text while `materialIndex` only contains references. Re-reading for concrete uncertainty, source changes, output verification or compaction remains allowed.

Protected material paths include `.case-agent`, `.git`, `.pi`, `.agents`, `.codex` and `.claude` at any depth, case-insensitively. Ordinary `AGENTS.md` can be explicitly selected as an instruction source. These checks are not secret scanning or an OS sandbox.

Immediate discoveries, prerequisite amendment, same-session repair and independent integration have completed specified local-model scenarios. Fixed-version comparisons nevertheless showed no quality or cost advantage for CASE, and later diagnostics include correct artifacts that failed overall acceptance. A successful stage does not cancel a later failure. See the [evidence index](READINESS.md#證據索引依問題查閱不串成一次成功) for the individual reports; this guide does not duplicate their evolving checkpoint history.
...
The authoritative case record is `.case-agent/cases/<UUID>/state.json`; run artifacts preserve responses, session/model information, usage and tool observations. Missing costs stay unknown/null. Inputs and submitted deliverables are bound to SHA256 versions. Submission is not verification: a different review session checks actual artifacts, and integration must pass every global criterion. Required context is never silently truncated: `maxChars` is a character budget, and excess returns `CONTEXT_TOO_LARGE`. Required inputs may use `delivery:"indexed"` and `purpose` to retain their version and reading obligation without inlining all content; use paginated reads. Read limits are 1 MiB per file and 200 lines; the model-visible `CASE_READ` receipt plus body is limited to 24,000 UTF-16 units, not tokens or bytes.

Read receipts identify the source version, actual line range, next page, empty files and out-of-range requests. EOF is not whole-file coverage: `wholeFile:true` applies only to a single read from the beginning returning all text. Coverage cannot be carried across source versions. CRLF-to-LF normalization and trailing empty line elements are retained without summarization or silent truncation. Reduce the number of lines for oversized multi-line results; `LINE_TOO_LONG` means a single line cannot fit. Character pagination is not provided; any necessary preprocessing requires existing authority and must not change frozen sources.

Optional run evidence now includes `traceVersion:1`: paired tool/request/compaction events, source versions and loaded-policy scope/hashes. Metadata is capped at 4 KiB per event, 128 events and 256 KiB per session; overflow or unmatched events are explicitly incomplete. It does not copy bodies or assume exact text survives compaction. Legacy observations may still contain private content. Trace is returned at session end, not guaranteed durable across process crashes. Persistence failure stops dispatch and exposes the original operation error, separate `persistenceErrors` and the available in-memory `run`; it never authorizes replaying side effects. Phase budgets and a stagnation controller are not provided by this change.

The SDK compactor retains a recent tail of one quarter of the model context window (capped at 20,000 tokens), with half the window reserved (capped at 16,384). For 32K this is 8192/16384; unknown windows retain SDK defaults. Session evidence records these settings. They are engineering settings, not exact token or summary-quality guarantees. After SDK recovery, an unaccepted response still ending with `length` is reported as `MODEL_OUTPUT_TRUNCATED`, not retried as ordinary JSON syntax. Costs, partial artifacts and the original task budget remain intact; compaction does not prove task completion.

Pi worker handoffs default unspecified source delivery to versioned indexes. Explicit `delivery:"inline"` still attaches full text; explicit `indexed` remains indexed. Goals, constraints, acceptance and source-freshness checks remain intact, and presentation does not mutate stored packets. Portable CLI/core context retains its inline default; programmatic callers can request `{defaultDelivery:"indexed"}`. Required indexed material must still be read as needed, not silently skipped.
...
- The v1 record-keeping path does not automatically dispatch agents or execute tests. The v2 pi integration creates isolated sessions and can run explicitly approved checks; neither path launches models, grants authority, enforces an operating-system sandbox, or publishes your work.
