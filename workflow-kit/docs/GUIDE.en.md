...
**2.0.0-preview.1 — status as of September 10, 2026.** Review disputes, interruption recovery, aligned role guidance and bounded literal search are implemented; compaction recovery has been observed. This batch remains on the working branch, not merged or pushed. The full suite passed 302 tests with two Windows symlink-permission skips. The latest normal task was cancelled after 601.856 seconds at its 600-second limit: planner then worker, no deliverable, and no review or integration. Compaction restored the output allowance, but repeated reading without delivery remains unresolved; the core quality goal has not passed acceptance.

Use the preview for work needing versioned assignments, continuation or review records, not as a proven unattended worker. The latest run recorded 234,469 cumulative SDK tokens including cache, not peak context or complete cost. Source integrity and final write scope passed, while incorrect relative-path requests still triggered a tool-scope violation classification. The 100.361-second overflow compaction completed without aborting; request maxTokens recovered from 1955 to 1 to 4096, which does not establish complete semantic preservation. No unchanged rerun or release is used to hide the missing deliverable. [Current validation and historical evidence](READINESS.md) separates the latest result from the earlier 332-second failure.
