...
**Feedback without changing the goal:** workers return `{"changeRequest":{"reason":"specific prerequisite or plan gap"}}` when existing data/authority can resolve a gap, or `{"blocked":{"reason":"missing external data"}}` when they cannot. A fresh planner returns a complete replacement `packets` plan, `rerunPacketIds`, and `reason`; core action `amend_plan` checks authority, dependencies and acceptance coverage. Valid unrelated verified work is retained; changed, overlapping or dependent work is invalidated. Integration failures require actual rework, not repeatedly asking a reviewer until it says yes. At most two automatic replans across runs share the original budgets. Unknown side effects still require explicit recovery.
...
Pi child sessions now submit their structured reply via `case_result` with `{result: object}` after completing all other tools. Further tool calls are rejected. This separates narrative from machine-readable handoff; core validation remains required. Raw final text and the transport used are retained. Other integrations without this tool still return strict JSON; the tool is internal, not an additional user setup step.
