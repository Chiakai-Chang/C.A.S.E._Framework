...
**2.0.0-preview.1 — status as of September 11, 2026.** Missing-file feedback and terminal compaction cancellation are implemented, including a controller created after cancellation. The full kit suite passed **320 tests with two Windows symlink-permission skips**; the package preview contains 36 files. The batch remains on its working branch, not merged or pushed.

A separate imported-artifact diagnostic completed review and integration in **291.427 seconds**, using **194,189 additional cumulative SDK tokens including cache**. Both roles read required materials, preserved the artifact and sources, and had no compaction events. Each still emitted one immediately failing post-result request event; no zero-charge claim is made. The reviewer confused edited-install rejection with version mismatch in its explanation, although the artifact value was correct; the integrator cited the correct edited-install check. This is not a native continuation, a normal end-to-end success or a causal comparison. No missing-file or acquisition-gate rejection occurred, so model recovery from those errors remains unproven. See the [latest evidence and diagnosis](../../docs/evaluation/2026-09-11-review-boundary-report.md).

The subsequent normal run from blank output **failed after 309.457 seconds**: the planner exhausted 16 turns with 34 successful read/list/search calls, no assignment and no artifact. It used **293,885 cumulative SDK tokens**, bringing this batch's two runs to **488,074** including cache. Active-session overflow compaction took 72.709 seconds; no compaction followed the turn-limit stop. The next focus is the initial planning handoff, not more identical full-workflow retries or a larger budget. Use the preview for versioned assignments, continuation or review records, not as a proven unattended worker. [Current validation and historical evidence](READINESS.md) keeps previous failures and costs separate.
