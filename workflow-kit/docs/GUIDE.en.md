...
Use the preview for versioned assignments, continuation or review records, not as a proven unattended worker. The normal run used 489,391 cumulative SDK tokens including cache; the separate repair diagnostic used 29,032 more. No oracle was supplied to either model session. Source/scope checks passed, but those do not establish semantic correctness. The successful repair used a newly reconstructed worker context and the pre-Unicode-boundary-fix tool version; the final boundary fix is covered by regression tests, not another full model run. [Current validation and historical evidence](READINESS.md) keeps each result and cost separate.
