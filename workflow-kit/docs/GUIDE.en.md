...
**2.0.0-preview.1 — status as of September 10, 2026.** Pi now defaults unspecified source delivery to versioned indexes while preserving explicit inline material. The full suite passed 304 tests with two Windows symlink-permission skips. This batch remains on the working branch, not merged or pushed. The latest normal workflow produced a fully correct artifact, but was cancelled during independent review at the original 600-second limit; total time including cancellation cleanup was 644.033 seconds. Integration did not run, so overall acceptance still failed.

Use the preview for versioned assignments, continuation or review records, not as a proven unattended worker. The latest run recorded 341,289 cumulative SDK tokens including cache, not peak context or complete cost. Source integrity, final write scope and recorded tool-path/policy checks passed. Planning to execution took about 315 seconds and execution to review about 221 seconds, including handoffs. A separate lean worker completed the original artifact in 149.441 seconds, but its budget and submission checks differed; this is neither a causal comparison nor full-workflow success. No oracle feedback or synthetic verdict was supplied to the normal workflow. [Current validation and historical evidence](READINESS.md) preserves all failures and the earlier confirmed compaction recovery separately.
