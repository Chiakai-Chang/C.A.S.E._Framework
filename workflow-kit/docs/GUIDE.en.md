...
**Latest September 11 follow-up:** The acquisition gate below is implemented; the full suite passed 314 tests with two Windows permission skips. A separate imported-artifact review read every required file but exhausted 16 turns after 170.202 seconds, without a verdict or integration (218,143 additional cumulative SDK tokens including cache). No gate rejection was attempted in that model run; rejection/recovery is covered by engineering tests, not proven model recovery. This is not a successful native continuation or complete workflow. Earlier results below remain historical evidence.

**2.0.0-preview.1 — status as of September 11, 2026.** The latest normal workflow produced a fully correct artifact but stopped after 454.159 seconds because a wrapped reviewer reply reached state storage without role validation. Reviewer tool and final-text replies now have structural validation and use the existing same-session correction budget; custom session returns are checked too. The full suite passed 311 tests with two Windows symlink-permission skips. This fixes the format boundary, not factual verification: the actual reviewer made no reads yet claimed checks with incorrect values. No integration completed, and the fix has not had a new normal local-model run. The earlier 40.206-second localized repair remains a separate diagnostic. This batch is not merged or pushed. See the [latest evidence and diagnosis](../../docs/evaluation/2026-09-11-review-boundary-report.md).

Use the preview for versioned assignments, continuation or review records, not as a proven unattended worker. The September 11 normal run used 227,033 cumulative SDK tokens including cache. The earlier whole-file-repair run used 489,391 and its separate localized repair diagnostic used 29,032 more. Source/scope checks do not establish semantic correctness. [Current validation and historical evidence](READINESS.md) keeps results and costs separate.
