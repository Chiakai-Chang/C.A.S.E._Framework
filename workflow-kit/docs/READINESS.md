...
更新：2026-09-12。此頁區分工程行為、模型結果及交付狀態；測試數不代替模型效果。套件仍為 **2.0.0-preview.1**；Git 交付見 [STATUS](../../docs/STATUS.md)。

## 目前結論

**最新完整本機任務通過**：固定 `94935d6`，既有多來源訂單案例從空白成果完成兩個相依工作包、六個獨立角色 session；361.349 秒、159,925 累計 SDK tokens，人工介入 0。外部評分核實所有欄位、來源及範圍；未插入答案、故障或額外檢查命令。這不是新盲測、成功率或省成本比較，也不能推翻下列誤驗反例。[實測與限制](../../docs/evaluation/2026-09-12-push-acceptance-report.md)。

最新工程回歸：353 項，351 通過、2 項權限跳過。真 pi SDK 0.84.2 搭配替代 HTTP 確認原生連線錯誤正確分類、零格式補問；未呼叫模型。此結果不覆蓋下列模型語意誤驗。

安全接續新增：僅 SDK 確認唯讀且成功清理的核對／整合明確連線錯誤，自動換 session 一次；原總預算及跨 run 額度不重設。SDK→runner 受控生成測試確認最後成果完成、不重做 worker、失敗成本保留。未知副作用、取消、語意否決及檢查命令不重播。這不是本機模型效果驗證。[完整規則](../../docs/design/2026-09-11-goal-loop-convergence.md)。

9/11 後續工程：過期卷宗不再先啟動模型；包含真實命令返修、整合中斷與重新載入接續的測試，全部 kit 為 336 通過、2 項權限跳過。角色回覆使用受控替身，不是新的模型成功證據；以下誤驗反例仍成立。[接續覆蓋與計畫](../../docs/design/2026-09-11-goal-loop-convergence.md)。

**保留的不同正常任務反例：失敗且誤驗完成。** `154edb3` 在 **343.108 秒／220,286 累計 tokens** 完成四角色並標 completed，但 nativeAutomaticHosts 漏掉 pi；worker、reviewer、integrator 均未排除錯誤。7 個接受引用真實，4 次假 ID 被拒收後修正，並不代表語意合格。來源未變、沒有額外產物；兩次畸形 path 請求被記為工具範圍違規、均未讀到內容。另有前題完整來源雜湊出現在本次假 ID，根因待查。[原始證據、離線核實與下一步](../../docs/evaluation/2026-09-11-holdout-journey-report.md)。

後續引用改善：pi 核對／整合改用當前 session 讀取回條 ID，由程式解析真實來源，拒絕未知／過期引用；判斷文字仍可能有錯。另一個預建缺陷案例的單獨 integrator **42.792 秒、10,066 tokens** 正確一負一正，引用核實、來源未變；不是正常整案或省成本比較。模型診斷後的錯誤指引及 legacy 相容小修另以回歸確認。[完整脈絡與成本](../../docs/evaluation/2026-09-11-receipt-review-report.md)。

先前正常主案例 **通過**：pi 執行交辦改為直接工作說明後，從空白成果完成四個獨立角色，**547.780 秒**、成果所有欄位與來源／範圍檢查通過，人工介入 0。這只證明該任務可完成；當時核對說明仍有引用錯誤。見[歷史成功與限制](../../docs/evaluation/2026-09-11-worker-grounding-report.md)。下表保留先前交付紀錄，本次反例以上方為準。

| 面向 | 已知結果與限制 |
|---|---|
| 工程回歸 | 最新完整 kit **353 項：351 通過、2 項 Windows 符號連結權限跳過**；新增 review-evidence 模組，由 integrations 封裝規則納入 |
| 直接執行交辦診斷 | 原完整任務單獨 SDK worker **169.286 秒**正確產出，來源保留，**120,354 SDK 累計 tokens**；無壓縮／誤交計畫。另保留啟動腳本漏接回呼的零 tokens 失敗；不等於正式完整流程通過 |
| 本次交辦改善 | pi 初次／返修使用直接執行的分段說明；共識、發現、全文、版本索引及額外欄位保留，呈現後仍檢查字數上限；核心 JSON 不變 |
| 局部返修診斷 | 新工具 case_edit 實際被模型使用，**40.206 秒**修好原缺陷且保留全部正確內容；新 worker 診斷，不是整案接續或原預算成功 |
| 先前正常主案例 | **547.780 秒、454,646 SDK 累計 tokens**，planner／worker／reviewer／integrator 完成，沒有壓縮。仍需不同任務驗證，不把單次通過當成功率 |
| 來源與工具範圍 | 最新正常任務的來源完整性、最終寫入範圍、記錄中工具路徑、政策及證據檢查均通過 |
| 歷史成本 | 先前正常流程 **227,033 SDK 累計 tokens**（含 cache），不是峰值 context 或全部成本。當時已交回結果後仍壓縮，本批補上停止收尾；不將額外診斷成本藏入原案 |
| 新舊成本分列 | 本批診斷 120,354 + 正常任務 454,646 = **575,000 SDK 累計 tokens**（含快取）；另有零 SDK tokens 啟動失敗。先前兩次規劃／失敗接續 489,969 及更早 488,074 均保留，不算成同一旅程或省成本證據 |
| 審查異議 | 工程已實作引用與版本檢查、一次重核及中斷保護；先前受控模型診斷未通過，最新正常任務沒有走到異議分支 |
| 核對格式修復 | 工具／最後文字與自訂 session 邊界拒絕包錯層及缺欄位；SDK 沿用同 session 格式修正。工程反例通過，真實模型自修尚未驗證 |
| 未解缺口 | 已以結構化回條引用處理來源識別，但語意相關性與所有核對文字並未受機械驗證；成本高，新介面的不同正常整案及跨任務穩定性尚未建立。歷史計畫矛盾與角色混淆不能因單次通過當成已根除 |
| 使用建議 | 短工作直接完成；需要版本化交辦、接續或獨立核對時再用完整流程。不建議無人監督執行重要工作 |

先前 309.457 秒未交辦與匯入成果診斷見[核對格式與證據缺口](../../docs/evaluation/2026-09-11-review-boundary-report.md)，更早[正常流程證據](../../docs/evaluation/2026-09-11-edit-journey-evidence.json)仍保留。先前返修回歸與局部修復見[局部修改報告](../../docs/evaluation/2026-09-10-localized-repair-report.md)。沒有把評分器交給模型；額外診斷與原預算分開。先前正確成果但整案失敗見[索引交辦復盤](../../docs/evaluation/2026-09-10-worker-focus-report.md)。
