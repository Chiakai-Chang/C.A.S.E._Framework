...
| 面向 | 已知結果與限制 |
|---|---|
| 工程回歸 | 完整 kit **306 項：304 通過、2 項 Windows 符號連結權限跳過**；封裝預覽 36 個檔案 |
| 本次交辦改善 | pi 未指定 delivery 的來源預設索引；顯式 inline、必讀責任與來源過期保護保留，可攜核心／CLI 預設不變 |
| 單獨執行診斷 | 完整原任務 149.441 秒產出正確成果並提交；不同回合上限與檢查設定，不構成因果比較，也未走拒收修復 |
| 最新正常任務 | planner → worker → reviewer；**成果全部欄位正確**，但核對期間達原 600 秒上限取消，收尾後 **644.033 秒**，未完成整合 |
| 來源與工具範圍 | 最新正常任務的來源完整性、最終寫入範圍、記錄中工具路徑、政策及證據檢查均通過 |
| 成本 | **341,289 SDK 累計 tokens**（含 cache），不是峰值 context 或全部成本；未回報用量未知。規劃至執行約 315 秒、執行至核對約 221 秒，含交接成本 |
| 審查異議 | 工程已實作引用與版本檢查、一次重核及中斷保護；先前受控模型診斷未通過，最新正常任務沒有走到異議分支 |
| 未解缺口 | 正確成果已產出，但全流程尚未完成。規劃重複解題及格式修正占用時間，不把單階段通過當成核心品質目標驗收完成 |
| 使用建議 | 短工作直接完成；需要版本化交辦、接續或獨立核對時再用完整流程。不建議無人監督執行重要工作 |

最新結果見[單獨執行與索引交辦復盤](../../docs/evaluation/2026-09-10-worker-focus-report.md)、[正常流程證據](../../docs/evaluation/2026-09-10-indexed-default-evidence.json)及[固定 manifest](../../docs/evaluation/2026-09-10-indexed-default-manifest.json)。沒有把評分答案或合成否決交給正常流程；原預算未放寬。
