...
- 新的持續任務：先確認成果、可觀察的驗收與不能違反的約束，再使用下列開始方式。
- 接續／即將壓縮 context：讀 [接續與 CLI](references/state-and-resume.md)。
...
核對每項驗收，執行與變更風險相稱的檢查。`record` 的 pass 是有來源的觀察，不代表證據內容經 CLI 驗真；不得以測試數或 agent 自述代替成果。需要使用者核准的條件，收到核准才記為 pass。
