...
assert.ok(['minimal','loaded','full-task','full-checked'].includes(variant),'Unknown variant');
...
const fullTask=variant.startsWith('full-'),checked=variant==='full-checked',outputName=fullTask?'adoption-map.json':'constants.json';
const maxTurns=checked?16:8;
...
const inputHashes=Object.fromEntries(fs.readdirSync(project).map(n=>[n,hash(fs.readFileSync(path.join(project,n)))]));
const report={kind:'worker-focus-probe/1',variant,status:'starting',project,prompt,source,sourceSha256:hash(source),codeHashes,inputHashes,
  configuration:{thinkingLevel:'off',contextWindow:32768,maxTokens:4096,maxTurns,maxDurationMs:180000,semanticPreflight:checked},requests:[],
