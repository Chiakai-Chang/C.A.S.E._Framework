...
Round-seven method hardening makes concurrent grading independent of transcript order: the audited publication trace must first identify one winner and one loser, and only the loser terminal verdict is adjudicated. Loser `SUCCESS` takes false-success priority over winner-side or earlier `DETECTED` text; without a unique topology the record is ineligible. Runner and verifier also share a canonical no-leading-zero `r7+` parser, v4 binds protocol/CLI/artifact source to one exact commit, and future manifest statuses remain within the existing closed enum. No raw record, selected aggregate, or decision changed, and no model was run.

Round-eight closes the topology and status edges found by final review. Concurrent grading now accepts only two exact canonical publish commands, one from each named actor, with terminal non-null exits and exactly one winner/loser; extra, approximate, duplicate, unknown, timed-out, pending, or internally contradictory publication traces are ineligible, as is a loser without one unique terminal verdict. Future canonical identity is resolved before outcome status: identity mismatch is `invalid-method`, every r7+ non-complete or M0 observation is `invalid-run`, and only adjudicated complete B0 may be `eligible-post-pilot`. An actual manifest-building test validates those writer outputs against the closed manifest schema. Historical statuses and all raw evidence remain unchanged; no model was run.
