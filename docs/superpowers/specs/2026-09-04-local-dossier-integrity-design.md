...
The in-process zero-network audit starts before case schema/dependency setup and follows case-causally-created asynchronous resources through final assertions. Before corpus loading, the runner probes the active Node 24 runtime for the exact lifecycle observations on which the audit depends: Promise-parent correlation on the same Promise resource observed by `async_hooks`, public timer/immediate cancellation followed by destroy at one deterministic checkpoint, and `after` delivery for a completed Immediate. Node 24.19.0 is the measured baseline; every other Node 24 release must pass this capability probe and is not represented as pre-verified. A failed probe closes the run before any case receives credit.

Network resource initialization is an immediate failure, and case-created runnable timers, persistent handles, unfinished one-shot work, or unresolved Promise continuations registered by the case must be quiescent at return. A bare unresolved Promise with no registered continuation is not pending work. Timer and immediate liveness is sampled after one deterministic, non-timed event-loop lifecycle checkpoint: a public cancellation whose runtime destroy notification has arrived is no longer pending, while mutable private handle fields carry no authority. A completed Immediate is removed on its `after` lifecycle event only after any child resource has inherited its case-causal identity. Cleanup uses module-loaded `node:timers` cancellation functions with the actual resource objects; it neither coerces timer objects nor trusts globals that a runner port can replace.

After `onCaseAssertions` and the isolated lifecycle checkpoint, the runner re-evaluates outcomes, the read-only derived view, the final repository tree, and the harness-owned Git baseline. It rebuilds binding facts only from that second evaluation; a port cannot retain credit from an earlier snapshot that it subsequently changed. The audit hook is disabled before the library call returns, including after detecting an unresolved continuation: this is detection, not a claim that the runner can cancel a Promise or sandbox a later side effect. Resources created outside the case scope are baseline resources and are not attributed to it. Cleanup is limited to safely recognized case-created Node resources; unknown resources remain failures without a claim that the library can close them. The formal conformance command writes its final result synchronously and terminates its process explicitly. This audit does not claim to sandbox network traffic initiated inside arbitrary child processes; the public subprocess vector proves only its declared CLI result.
