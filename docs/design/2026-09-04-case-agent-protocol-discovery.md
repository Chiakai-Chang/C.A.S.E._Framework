...
**Implementation evidence**: the controlled-test corpus at evaluated implementation commit `0a1f6af9d8385061d3838808083648300422353a` exercises 140 cases with no uncovered required direction, including real bundled human help. Controlled-test remains an injected capability profile, not a production platform claim; it does not establish a working Windows or POSIX production adapter.

**Baseline evidence**: the frozen protocol is commit `c7e1e083065e62956055286c03b4bd8564e729d2`. The first pilot and every r1-r5 attempt remain immutable history but are excluded from selected evidence: r3's disconnected publication method and missing raw responses made its earlier 3/4 result irreproducible, while r4 had unverifiable model provenance and an incomplete process boundary. The selected method-frozen r6 run used the authorized local Qwen3.8 27B model and a case-specific deterministic post-hoc adjudicator: B0 detected all four cases with zero false-success outcomes. All four paired public Windows M0 runs remained invalid because initialization correctly returned `CASE_E_UNSUPPORTED_PROFILE` before the target behavior could run.

**Measured burden**: selected r6 B0 records contain 35 trace events: 32 model-selected actor commands plus three frozen evaluator injections. They used 235,693 ms, 34,925 input tokens, 5,851 output tokens, and no user decisions, corrections, or recovery steps. M0 burden is not comparable: its four initialization commands and 2,276 ms measure only early unsupported-profile rejection, with unavailable token counts.

**Quality and cognitive-load assessment**: canonical revisions/digests and the conformance oracle remain promising for making stale state mechanically visible, while the corrected r6 B0 result shows the baseline model can also detect these four failures under this exact post-pilot method. A useful comparative user outcome is therefore still unproven because no production adapter can execute the paired M0 cases on this host. Adding prompts, roles, plugins, or ceremonies now would increase cognitive and maintenance burden without closing that gap.
