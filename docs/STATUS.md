...
更新：2026-09-06。`2.0.0-preview.1` 的 v2 核心與 pi 自動 context 流程已實作，本輪分支尚未合併或推送。採用入口：[v2 指南](../workflow-kit/docs/V2.md)、[原生 pi 安裝](../workflow-kit/docs/HOSTS.md)。

本輪工程驗證 110/110 通過，但新增回饋流程的四次本機模型探測皆失敗，最後一次反覆讀取而未寫出產物，耗盡時間預算。[完整報告與下一步](evaluation/case-feedback-report.md)保留失敗及成本；目前不能宣稱已實現可靠完成工作的目標。

2026-09-06：在相同預覽分支補齊版本化專案共識、契約不變的 amend_plan、worker 缺口回報、整體失敗後補做、必要材料索引及人類確認的可執行檢查。這不是又一套平台；舊成果在相依與來源仍有效時保留。功能及本機模型探測的最新結果集中於 READINESS。三組效果比較、長 context 壓力、跨工具實際交接與跨任務經驗重用仍未完成，不能宣稱原始構想所有效果均已驗收。
