...
本次工作分支另已補上可修正的計畫引用回饋，以及 worker 專用 `case_edit` 局部修改，保留既有範圍與來源版本。**本批保留工作分支，未合併／推送**。完整工程回歸 **307 通過、2 項 Windows 符號連結權限跳過**。最新正常流程在返修時漏掉原本正確內容，601.698 秒取消，整案及最終成果均未通過；後續新版本的局部返修診斷約 40.206 秒成功，不能混為整案完成。

本次規劃約 46 秒、首次執行約 130 秒，但核對及返修仍有品質與成本問題。新的局部修改工具在實際模型中保留了所有正確內容、修好原缺陷；下一項未完成驗收是接回完整核對與整合。不再原樣從頭抽樣，也不把新增診斷成本藏入舊案。先前正確成果但逾時的歷史仍保留，不以新失敗覆寫。

[最新實測與裁定](evaluation/2026-09-10-localized-repair-report.md) · [集中驗證範圍](../workflow-kit/docs/READINESS.md) · [接續計畫](superpowers/plans/2026-09-10-recovery-and-delivery.md)
