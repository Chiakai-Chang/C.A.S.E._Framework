...
後續已接上安全唯讀連線恢復：符合條件的核對／整合連線階段錯誤會在原預算內自動換 session 一次，保留失敗及成本，不重做 worker；跨 run 不補額度。真 pi SDK 替代 HTTP 探測亦確認原生錯誤不再誤走 JSON 補問，未新增本機模型效果證據，語意誤驗仍未解決。[適用條件與限制](design/2026-09-11-goal-loop-convergence.md)。

9/11 後續閉環修補：已防止過期卷宗仍啟動模型；真實命令驗證錯誤成果拒收、修正、整合中斷及磁碟接續完成，未重做有效寫入。全部 kit 測試 336 通過、2 項權限跳過。沒有新增本機模型測試，以下語意誤驗仍未解決。[覆蓋復盤與後續順序](design/2026-09-11-goal-loop-convergence.md)。
