...
**最新不同正常任務驗收失敗，出現假完成。** 固定 `154edb3` 從空白成果完成四角色，卷宗雖為 completed，但一項欄位錯誤遭兩層核對放行。**343.108 秒、220,286 tokens**；7 個引用都真實，卻不保證推論正確。另有前題完整雜湊出現在捏造 ID 的異常，尚未證明來源或快取根因。本輪沒有改產品程式、沒有延長預算或重抽；仍未合併／推送。[完整反例與下一步](evaluation/2026-09-11-holdout-journey-report.md)。

接續改進：核對／整合由模型提交回條 ID、程式解析實際來源，拒絕假引用及過期引用，保留原始回覆。另一個已知缺陷的單獨 integrator 診斷 **42.792 秒、10,066 tokens** 正確一負一正；完整工程現為 **333 通過、2 項權限跳過**。不是新的完整旅程或省成本證明；仍未合併／推送。[引用改善及成本復盤](evaluation/2026-09-11-receipt-review-report.md)。

前次將 pi 初次及返修交辦改為直接執行的分段說明，保留共識、回饋、驗收及材料；核心 JSON、權限與預算不變。不改模型服務或全域 pi。

**先前正常主案例通過**：從空白成果完成 planner → worker → reviewer → integrator，**547.780 秒、454,646 SDK 累計 tokens**，成果全部欄位、來源完整性及範圍檢查通過，人工介入 0。另有單獨執行診斷 169.286 秒成功、120,354 tokens，以及一次診斷腳本啟動失敗（零 SDK tokens）；該批合計 **575,000 tokens**，皆含快取。不是成功率或省成本證明，也不能覆蓋本次反例。

仍有已知限制：先前核對文字的引用錯誤促成本次介面修正，但新介面不保證語意相關性或所有敘述正確；成本高，泛用性、異議真實路徑及其他 AI 工具原生整合未因此通過。下一步追查跨題雜湊的輸入來源及語意漏驗，不反覆抽同一題。先前七處錯誤及 489,969 tokens 失敗批次仍見[歷史報告](evaluation/2026-09-11-initial-planning-report.md)，不覆寫成成功。整體交付仍為預覽版。
