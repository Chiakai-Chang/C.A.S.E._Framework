...
- B0: read `CASE.md`, read `artifact.txt`, inspect `git status`/`git diff`/`git rev-parse`, edit `artifact.txt`, and commit through Git.
...
Both writers start from the identical recorded version. Their mutation attempts are released concurrently; no evaluator retry is added.
