...
The runner is a closed evaluator for its explicit Git operations and the packaged CLI init command; it is not a general process sandbox. Git hooks, signing, interactive credentials, and credential helpers are disabled for evaluator-launched Git. Fetches have abort signals, child commands retain partial output and have bounded deadlines, and the runner records timeout before making a detection decision. On Windows, `taskkill /T /F` is used only for normal descendants; no Windows Job Object is available in the current dependency-free Node implementation, so independently detached or adversarial escape processes are outside the claim. POSIX process groups are best-effort. Formal runner exit remains bounded by the absolute per-case deadline and cleanup; arbitrary hostile subprocess containment is unclaimed.
