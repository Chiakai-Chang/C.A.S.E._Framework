...
Task 12: fix round 8 base: `f421bdf`.
Task 12: Ruling: EVAL-M0-001 eligibility requires a closed two-entry publication topology and one unique terminal verdict from the exact loser; approximate commands or ambiguous actors/exits cannot be interpreted into evidence.
Task 12: Ruling: canonical future identity is classified before future outcome. All r7+ outputs use only `eligible-post-pilot`, `invalid-method`, or `invalid-run`; historical r1-r6 statuses remain unchanged.
Task 12: fix round 8 implementation complete pending independent re-review (34/34 closed result/manifest validation, 310/310 core tests, 60/60 evaluation tests, 140/140 conformance, 86-file package; no model run and no raw record rewrite).
Task 12: fix round 8 independent review found and the same round addressed future-v4 historical-suffix bypass, noncanonical `git push` publication attempts, and identical-revision conflict traces; targeted regressions are green before final full-gate repetition.
Task 12: fix rounds 9-10 completed through `0a1f6af`; external integrity revision-prefix matching now rejects ambiguous shared prefixes and accepts only a full hash or the shortest unique 7-39 character prefix, with case-normalized comparison. Raw evaluation outcomes were not rewritten and no model rerun occurred.
Task 12: complete at `0a1f6af` after independent methodology and runtime/integrity reviews both returned clean; exact full verification passed 310/310 core tests, 63/63 evaluation tests, 140/140 conformance cases, 34/34 external integrity records, the 86-file package allowlist, range diff checking, and a clean worktree.

Final whole-branch review: Standards axis found no hard issue and two deferred judgment calls (duplicated workflow request validation and the oversized conformance runner). Spec axis found one hard issue: the living design checkpoint still repeated withdrawn r3 measurements. The checkpoint was corrected to the selected r6 evidence and the same reviewer returned clean. The two judgment calls are retained as post-M0 maintainability debt because refactoring them now would expand risk without changing the evidence gate.
Next: repeat final verification from the review-fix commit, preserve the branch without publishing, and apply the finishing-a-development-branch decision gate.
