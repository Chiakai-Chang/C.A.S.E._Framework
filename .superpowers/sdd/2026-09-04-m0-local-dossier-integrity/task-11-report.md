...
The audit now tracks unresolved Promise resources created in the case scope, while explicitly excluding the still-running corpus body and current runner continuation. The Promise mutation receives its red summary while the gate is still unresolved, then performs a second complete clean run against the same corpus and obtains **139/139** before the gate is released; this proves the first invocation disabled its hook and did not contaminate the next run. The test finally resolves the gate and lets its DNS callback complete, but neither implementation nor report claims that `runCorpus` cancels the Promise or audits/sandboxes that later side effect after return.

Cancellation is modeled from lifecycle state, not elapsed time. On the pinned Node 24 runtime, `clearTimeout` and `clearImmediate` synchronously mark their resource destroyed even if the async-hooks `destroy` notification has not run; such a handle has no runnable work and is removed from the pending set. No grace period or fixed sleep was added.
